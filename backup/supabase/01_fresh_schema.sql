...
-- ============================================================================
-- Run this AFTER running 00_fresh_reset.sql
-- This creates a completely fresh database with all tables, functions, and policies
-- ============================================================================
...
CREATE TYPE transaction_type AS ENUM (
  'cash_in',              -- Customer gives cash → Send to their bank/wallet
  ...
  'account_transfer',     -- Move money between own accounts
  'loan_given',           -- Give loan to customer
  'loan_received',        -- Receive loan repayment
...
-- Accounts table (bank accounts, wallets, cash on hand)
...
-- ============================================================================
-- STEP 7: Create function to update account balances
-- ============================================================================

CREATE OR REPLACE FUNCTION update_account_balance()
RETURNS TRIGGER AS $$
BEGIN
  -- For cash_in: Receive cash, send from bank (Cash increases, bank decreases)
  ...
  -- For cash_out: Give cash, receive in bank (Cash decreases, bank increases)
  ...
  -- For cash_in_physical: Add physical cash (only cash increases)
  ELSIF NEW.type = 'cash_in_physical' THEN
    UPDATE accounts SET current_balance = current_balance + NEW.amount 
    WHERE user_id = NEW.user_id AND type = 'cash';
  
  -- For cash_out_physical: Remove physical cash (only cash decreases)
...
-- ============================================================================
-- STEP 8: Create function to update cash position
-- ============================================================================
...
BEGIN
  ...
  -- For cash_in
  IF NEW.type = 'cash_in' THEN
    INSERT INTO cash_positions (user_id, date, total_cash_received, total_fees_earned)
    VALUES (v_user_id, v_date, NEW.amount, NEW.fee_amount)
  ...
  -- For cash_out
  ELSIF NEW.type = 'cash_out' THEN
    INSERT INTO cash_positions (user_id, date, total_cash_given, total_fees_earned)
    VALUES (v_user_id, v_date, NEW.amount, NEW.fee_amount)
    ON CONFLICT (user_id, date)
    DO UPDATE SET 
  ...
  -- For cash_in_personal with physical subcategory
  ...
  -- For cash_out_personal with physical subcategory
...
-- ============================================================================
-- STEP 9: Create function to update daily summary
-- ============================================================================
...
-- ============================================================================
-- STEP 10: Create function to log activities
-- ============================================================================
...
-- ============================================================================
-- STEP 11: Create function to automatically create profile on user signup
-- ============================================================================
...
-- ============================================================================
-- STEP 12: Create view for dashboard summary
-- ============================================================================
...
-- ============================================================================
-- STEP 14: Create default Cash on Hand account for existing users
-- ============================================================================

-- This function creates a default cash account when a profile is created
CREATE OR REPLACE FUNCTION create_default_cash_account()
RETURNS TRIGGER AS $$
BEGIN
  INSERT INTO accounts (user_id, name, type, opening_balance, current_balance, provider, is_active)
  VALUES (NEW.id, 'Cash on Hand', 'cash', 0, 0, 'Physical Cash', TRUE);
  RETURN NEW;
END;
$$ LANGUAGE plpgsql;

-- Create trigger to add default cash account on profile creation
CREATE TRIGGER create_default_cash_on_profile
  AFTER INSERT ON profiles
  FOR EACH ROW
  EXECUTE FUNCTION create_default_cash_account();

-- ============================================================================
-- INSTALLATION COMPLETE
-- ============================================================================

SELECT '✅ Fresh database schema installed successfully!' as status;
SELECT '🎉 Your Money Trade Shop Management System is ready!' as message;
SELECT '➡️  Next steps:' as next_steps;
SELECT '   1. Create a user account in the app' as step_1;
SELECT '   2. Add your bank accounts and wallets' as step_2;
SELECT '   3. Add customers with their fee structures' as step_3;
SELECT '   4. Start recording transactions!' as step_4;
